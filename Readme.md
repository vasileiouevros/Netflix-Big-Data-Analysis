...
The main challenges in this project were related to the quality and structure of the dataset. The raw Netflix data contains missing values, inconsistent formatting, and mixed representations, particularly in fields such as duration (minutes versus seasons), date_added (free-text dates), and multi-valued attributes like genres and countries. These issues were addressed through explicit data cleaning, normalization, and schema design decisions. Missing categorical values were identified and filled with a consistent placeholder to preserve dataset integrity, while inconsistent formats were resolved by parsing free-text date fields into datetime objects and separating duration values into numeric components and units. Multi-valued attributes such as genres and countries were normalized and stored as arrays in a document-oriented MongoDB schema, enabling flexible querying and aggregation using $unwind. In addition, the dataset is static and metadata-only, which limited the applicability of streaming analytics and real-time processing. As a result, the project adopted a batch-processing approach using MongoDB aggregation pipelines, which was better aligned with the characteristics and scale of the data and supported efficient analytical queries.
